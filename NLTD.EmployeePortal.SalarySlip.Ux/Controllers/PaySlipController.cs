...
namespace NLTD.EmployeePortal.SalarySlip.Ux.Controllers
{
    public class PaySlipController : BaseController
    {
        ...
        public JsonResult SendMail(List<string> values)
        {
            ...
            string sendEmailOn = ConfigurationManager.AppSettings["SendEmailON"];
            ...
            if (paySlipList != null && paySlipList.Count > 0)
            {
                ...
                //Check the SendEmailON from WebConfig to send email.
                if (!string.IsNullOrWhiteSpace(sendEmailOn) && sendEmailOn.Trim().ToUpper() == "TRUE")
                {
                    sendEmail = true;
                }
